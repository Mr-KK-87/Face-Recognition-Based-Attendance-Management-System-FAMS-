...
from tkinter import Button, Label, ttk, Toplevel, messagebox
import cv2
import csv
import os
import numpy as np
...
def manually_fill():
    ...
    def fill_attendance():
        ...
        try:
            setup_conn = pymysql.connect(host='localhost', user='root', password='')
            setup_cursor = setup_conn.cursor()
            setup_cursor.execute("CREATE DATABASE IF NOT EXISTS face_reco_fill;")
            setup_conn.close()
        ...
        if subb == '':
            ...
            def create_csv():
                ...
                if 'cursor' not in globals():
                    print("Error: Database connection not established. Cannot export CSV.")
                    return
                if not cursor:
                    print("Error: Database connection not established. Cannot export CSV.")
                    return
                ...
                csv_name = 'C:/Users/prudhvi/OneDrive/Desktop/Chinni/VS Code/Face-Recognition_project/Attendance/Manually Attendance/'+DB_table_name+'.csv'
                csv_folder = os.path.dirname(csv_name)
            ...
            def attf():
                import subprocess
                subprocess.Popen(
                    r'explorer /select,"C:\Users\prudhvi\OneDrive\Desktop\Chinni\VS Code\Face-Recognition_project\Attendance\Manually Attendance\-------Check atttendance-------"')
        ...
        try:
            ...
            while (True):
                ...
                for (x, y, w, h) in faces:
                    ...
                    training_path = r"C:\Users\prudhvi\OneDrive\Desktop\Chinni\VS Code\Face-Recognition_project\TrainingImage"
                    os.makedirs(training_path, exist_ok=True)
                    cv2.imwrite(os.path.join(training_path, f"{Name}.{Enrollment}.{sampleNum}.jpg"), gray)
...
# Ensure directory exists
            student_csv_dir = r"C:\Users\prudhvi\OneDrive\Desktop\Chinni\VS Code\Face-Recognition_project\StudentDetails"
            os.makedirs(student_csv_dir, exist_ok=True)

            csv_file_path = os.path.join(student_csv_dir, "StudentDetails.csv")
            file_exists = os.path.isfile(csv_file_path)

            with open(csv_file_path, 'a+', newline='') as csvFile:
                writer = csv.writer(csvFile)
                if not file_exists:
                    writer.writerow(['Enrollment', 'Name', 'Date', 'Time'])  # header
                writer.writerow(row)
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                while True:
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        if (conf < 70):
                            ...
                            date = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d')
                            timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
                            expected_columns = {'Enrollment', 'Name'}
                            if not expected_columns.issubset(set(df.columns)):
                                raise ValueError(f"CSV file is missing required columns. Found columns: {df.columns.tolist()}")
                            aa_array = df.loc[df['Enrollment'] == Id]['Name'].values
                            aa = aa_array[0] if len(aa_array) > 0 else 'Unknown'
                            global tt
                            tt = str(Id) + "-" + str(aa)
                            En = '15624031' + str(Id)
                ...
                timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
                Hour, Minute, Second = timeStamp.split(":")
                attendance_dir = r"C:\Users\prudhvi\OneDrive\Desktop\Chinni\VS Code\Face-Recognition_project\Attendance"
                ...
                root.configure(background='grey80')
                cs = os.path.abspath(filePath)
                if not os.path.exists(cs):
                    print(f"❌ File not found: {cs}")
                    return
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"C:\Users\prudhvi\OneDrive\Desktop\Chinni\VS Code\Face-Recognition_project\Attendance\-------Check atttendance-------"')
...
def admin_panel():
    ...
    def log_in():
        ...
        if username == 'Kumaraswamy':
            if password == 'Kumar@117':
                win.destroy()
                import csv
                import tkinter
                root = tkinter.Tk()
...
def training():
    ...
    # Check if the TrainingImage folder exists and is not empty
    if not os.path.exists("TrainingImage") or not os.listdir("TrainingImage"):
        msg = 'Please make sure "TrainingImage" folder exists and has images'
        Notification.configure(text=msg, bg="red", fg="white", width=50, font=('times', 18, 'bold'))
        Notification.place(x=250, y=400)
        return
    ...
    try:
        ...
        msg = f"Error loading images: {e}"
        Notification.configure(text=msg, bg="red", fg="white", width=50, font=('times', 18, 'bold'))
        Notification.place(x=250, y=400)
        return

    if len(faces) == 0:
        msg = "No faces found in TrainingImage folder"
        Notification.configure(text=msg, bg="red", fg="white", width=50, font=('times', 18, 'bold'))
        Notification.place(x=250, y=400)
        return
    ...
    try:
        model_dir = r"C:\Users\prudhvi\OneDrive\Desktop\Chinni\VS Code\Face-Recognition_project\TrainingImageLabel"
        os.makedirs(model_dir, exist_ok=True)
        model_path = os.path.join(model_dir, "Trainner.yml")
        recognizer.save(model_path)

    except Exception as e:
        q = 'Failed to save model: ' + str(e)
        Notification.configure(text=q, bg="red", fg="white", width=50, font=('times', 18, 'bold'))
        Notification.place(x=250, y=400)
        return

    res = "Model Trained Successfully"
    Notification.configure(text=res, bg="olive drab", fg="white", width=50, font=('times', 18, 'bold'))
    Notification.place(x=250, y=400)
